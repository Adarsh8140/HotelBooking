	...
	<form action="" id="manage-check">
		...
		<div class="form-group">
			...
			<input type="text" name="contact" id="contact" class="form-control"
				value="<?php echo isset($meta['contact_no']) ? $meta['contact_no'] : '' ?>" required>
		</div>
		<div class="form-group">
		...
		date_default_timezone_set('Asia/Kolkata'); // Change this to your local time zone if needed
		?>
		<div class="form-group">
			<label for="date_in_time">Check-in Time</label>
			<input type="time" name="date_in_time" id="date_in_time" class="form-control"
				value="<?php echo isset($meta['date_in']) && !empty($meta['date_in']) ? date('H:i', strtotime($meta['date_in'])) : date('H:i'); ?>"
				required>
		</div>
